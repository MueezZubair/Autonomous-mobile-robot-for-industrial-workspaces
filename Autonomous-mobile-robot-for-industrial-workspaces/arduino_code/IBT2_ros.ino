...
void calc_pwm_values(const geometry_msgs::Twist& cmdVel) {
  ...
  if (cmdVel.linear.x < 0)
  {
    ...
    analogWrite(RPWM_Output, pwmLeftReq+10);
    analogWrite(LPWM_Output2, pwmLeftReq+10);
    analogWrite(RPWM_Output2, 0);
  }
  else if (cmdVel.linear.x > 0)
  {
    analogWrite(LPWM_Output, pwmLeftReq+10);
    analogWrite(RPWM_Output, 0);
    analogWrite(LPWM_Output2, 0);
    analogWrite(RPWM_Output2, pwmLeftReq+10);
  }
  else if (cmdVel.angular.z <  0)
  {
    analogWrite(LPWM_Output, pwmLeftReq+70);
    analogWrite(RPWM_Output, 0);
    analogWrite(LPWM_Output2, pwmLeftReq+70);
    analogWrite(RPWM_Output2, 0);
  }
  else if  (cmdVel.angular.z > 0)
  {
    analogWrite(LPWM_Output, 0);
    analogWrite(RPWM_Output, pwmLeftReq+70);
    analogWrite(LPWM_Output2, 0);
    analogWrite(RPWM_Output2, pwmLeftReq+70);
  }
  else if(cmdVel.linear.x ==0)
  {
    analogWrite(LPWM_Output, 0);
