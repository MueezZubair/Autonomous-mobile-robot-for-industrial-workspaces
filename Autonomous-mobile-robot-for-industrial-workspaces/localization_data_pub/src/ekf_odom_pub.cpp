...
const double TICKS_PER_REVOLUTION = 2950; // For reference purposes.
const double WHEEL_RADIUS = 0.06; // Wheel radius in meters
const double WHEEL_BASE = 0.41; // Center of left tire to center of right tire
const double TICKS_PER_METER = 7825; // Original was 2800
...
void Calc_Left(const std_msgs::Int32& leftCount) {
  ...
  static int32_t lastCountL = 0;
  if(leftCount.data != 0 && lastCountL != 0) {
         
    int32_t leftTicks = (leftCount.data - lastCountL);
...
void Calc_Right(const std_msgs::Int32& rightCount) {
  ...
  static int32_t lastCountR = 0;
  if(rightCount.data != 0 && lastCountR != 0) {
 
    int32_t rightTicks = rightCount.data - lastCountR;
     
    if (distanceRight > 1000000000) {
      distanceRight = (0 - (4294967295 - distanceRight))/TICKS_PER_METER;
    }
    else if (rightTicks < -1000000000) {
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber subForRightCounts = node.subscribe("/front_right/encoder", 100, Calc_Right, ros::TransportHints().tcpNoDelay());
  ros::Subscriber subForLeftCounts = node.subscribe("/front_left/encoder", 100, Calc_Left, ros::TransportHints().tcpNoDelay());
